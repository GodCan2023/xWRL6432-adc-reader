...
class AdcReader(threading.Thread):
    ...
    def run(self):
        ...
        while self._running:
            try:
                ...
                # print(f"1 frame : {time_interval_ms:.2f} ms")           
    ...
    def save_to_npz(self, file_path: str | Path, config_metadata: dict = None, recorded_frames: list = None) -> bool:
        try:
            file_path_str = str(file_path)

            # Stack frames into a single dimensional array
            frames_array = np.array(recorded_frames)

            # Prepare data dictionary for MATLAB
            data_to_save = {
                'dataMat': frames_array,  # MATLAB变量名
            }

            if config_metadata is not None:
                # Add config metadata to MATLAB dictionary
                data_to_save.update({
                    'nSample': config_metadata.get('num_adc_samples', 0),
                    'nRx': config_metadata.get('num_rx_ant', 0), 
                    'nTx': config_metadata.get('num_tx_ant', 0),
                    'nFrame': len(recorded_frames),
                    'periodicity': config_metadata.get('frame_period', 0) / 1000.0  # Convert ms to seconds
                })

            # Save as MATLAB .mat file
            
            savemat(file_path_str, data_to_save)
            
            return True
        except Exception as e:
            print(f"ADC Recorder: Error saving data to MAT file '{file_path}': {e}")
            return False
